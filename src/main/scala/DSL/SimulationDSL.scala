...
@main def DSLDemo(): Unit =
  ...
  Simulation()
    ...
    .agent("Runner").start(1,9).withLearner().goal(2,4).reward(100).end()
    .agent("Opener").start(8,1).withLearner().noGoal()
    .on("Opener",8,6).openWall(2,3).give(50)
    .episodes(10).steps(10)
    .delay(100)
    .withGUI(true)
    .play()