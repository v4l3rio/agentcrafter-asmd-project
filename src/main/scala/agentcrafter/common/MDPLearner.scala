 ...
 *
 * This class implements Q-learning using an MDP-based approach that separates the learning algorithm from the action
 * selection policy. It provides an alternative to the traditional QLearner with potentially different convergence
 ...
 * @param gridWorld
 *   The grid world environment for the agent
 * @param initialState
 *   The starting state for episodes
 */
class MDPLearner(
   ...
   *   The current state
   * @return
   *   The selected action and whether it was exploratory
   */
   ...
   *
   * @return
   *   An immutable map containing all state-action Q-values
   */
...
private class EpisodeRunner(
  ...
  def run(startState: State, maxSteps: Int, epsilon: Double, learningParams: LearningParameters): EpisodeOutcome =
    ...
    def loop(state: State, steps: Int, trajectory: List[(State, Action, Boolean, Array[Double])]): EpisodeOutcome =
      if state == goalState then (true, steps, trajectory.reverse)
      else if steps >= maxSteps then (false, steps, trajectory.reverse)
      else
