 ...
 * 
 * This trait defines the core contract for all learning algorithms in the AgentCrafter framework.
 * Implementations should provide Q-learning or similar value-based learning capabilities.
 */
trait Learner:
  /**
   ...
   * @return A tuple containing the chosen action and a boolean indicating if the action was exploratory
   */
   ...
   * 
   * @param state The state from which the action was taken
   ...
   * @param nextState The resulting state (typically the goal state)
   */
   ...
   * @return An EpisodeOutcome containing success status, step count, and trajectory
   */
   ...
   * 
   * @return A map from (State, Action) pairs to their Q-values
   */
