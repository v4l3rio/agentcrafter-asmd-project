...
class QLearner(
  ...
  /**
   * Represents a trajectory of states, actions, exploration flags, and Q-values.
   *
   * This is used to log the agent's experience during an episode.
   *
   * @note The trajectory is a list of tuples containing:
   *       - State before action
   *       - Action taken
   *       - Boolean indicating if the action was exploratory
   ...
   *         - Trajectory: trajectory log with (state, action, wasExploring, qValues) for each step
   ...
   * }}}
   */
  def episode(maxSteps: Int = 200, exploitOnly: Boolean = false): Option[(Boolean, Int, Trajectory)] =
    gridEnv.map { env =>
      ...
      var steps = 0
      var done = false
      val trajectory = ArrayBuffer.empty[(State, Action, Boolean, Array[Double])]

      while !done && steps < maxSteps do
        val (action, exploring) = choose(currentState)
        val (nextState, reward, isGoal) = env.step(currentState, action)

        if !exploitOnly then
          update(currentState, action, reward, nextState)

        val qValues = if useGridOptimization then
          QArray.get(currentState.r)(currentState.c).clone
        else
          A.map(action => getQValue(currentState, action))

        trajectory += ((currentState, action, exploring, qValues))
        currentState = nextState
        done = isGoal
        steps += 1

      (done, steps, trajectory.toList)
    }