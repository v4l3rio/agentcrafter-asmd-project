...
/**
 * Type alias to represent an agent's trajectory during an episode. Contains state, action, exploration flag and
 * Q-values for each step.
 */
...
object QLearner:
   ...
   * @param resetFunction
   *   Function that resets the environment to the initial state
 ...
 *
 * This class implements the Q-learning algorithm with epsilon-greedy exploration. It manages learning through episodes,
 * updating Q-values based on received rewards and observed state transitions.
 *
 * @param goalState
 *   The goal state that terminates episodes successfully
 * @param goalReward
 *   The reward given when the goal state is reached
 * @param updateFunction
...
class QLearner private (
   ...
   * @return
   *   Episode result (success, number of steps, trajectory)
   */
   ...
   *
   * @return
   *   Map containing all current Q-values
   */
   ...
   *   The current state
   * @return
   *   Tuple containing (action, is_exploration)
   */
   ...
   *
   * @return
   *   String containing configuration information
   */
   ...
   *
   * This recursive method implements the main episode loop, handling action selection, environment execution, Q-value
   * updates and trajectory recording.
   *
   * @param state
   ...
   * @param currentStep
   *   The current step count in the episode
   * @param trajectory
   *   The trajectory of the episode so far
   */
  @tailrec
