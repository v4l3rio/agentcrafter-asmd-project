 ...
 *
 * @param pos
 *   The state/position where the wall should be removed
 */
 ...
 *
 * Triggers activate when a specific agent reaches a specific location, causing one or more effects to be applied.
 *
 * @param who
 *   The ID of the agent that can trigger this effect
 * @param at
 *   The state/position where the trigger is located
 * @param effects
 *   List of effects to apply when triggered
 */
 ...
 * @param goal
 *   Optional goal position (if None, agent has no specific goal)
 * @param learner
 *   The Q-learning algorithm instance for this agent
 */
 ...
 * @param staticWalls
 *   Set of positions that are permanently blocked
 * @param triggers
 *   List of conditional triggers in the environment
 * @param agents
 *   List of agent specifications
 * @param episodes
 *   Number of episodes to run in the simulation
 * @param stepLimit
 *   Maximum steps per episode before forced termination
 * @param stepDelay
 *   Delay in milliseconds between steps (for visualization)
 * @param showAfter
 *   Episode number after which to start showing visualization
 */
case class WorldSpec(
  rows: Int,
  cols: Int,
  stepPenalty: Double,
