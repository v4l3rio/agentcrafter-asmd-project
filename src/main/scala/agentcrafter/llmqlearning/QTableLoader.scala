 ...
 *
 * This object provides functionality to convert Q-tables between their internal representation and JSON format,
 * enabling persistence, LLM integration, and data exchange between different components of the system.
 */
object QTableLoader:
   ...
   *
   * This method parses JSON data (potentially generated by an LLM) and injects the Q-values into the specified learner
   * using reflection. The JSON is cleaned of common LLM decorations before parsing.
   *
   * @param raw
   *   Raw JSON string containing Q-table data
   * @param learner
   *   The learner instance to load the Q-table into
   * @return
   *   Success if loading succeeded, Failure with error details if it failed
   */
   ...
   *
   * @param learner
   *   The learner whose Q-table should be serialized
   * @return
   *   A pretty-printed JSON string representation of the Q-table
   */
