 ...
 *   1. Generate content from LLM using a prompt and simulation context 2. Process the LLM response to extract the
 *      relevant content 3. Load the processed content into the simulation builder
 *
 * @tparam T
 *   The type of content this service generates (e.g., String for walls, JSON for Q-tables)
 */
trait LLMService[T]:
  ...
  /**
   * Builds the complete prompt for LLM generation. Default implementation uses the prompt as-is, but can be overridden
   * for custom prompt building.
   *
   * @param builder
   *   The simulation builder (for context)
   * @param userPrompt
  ...
  /**
   * Processes the raw LLM response to extract the relevant content. This method should be implemented by concrete
   * services to handle their specific response formats.
   *
   * @param response
   *   The raw LLM response
   * @return
   *   Some(content) if extraction successful, None otherwise
   */
  protected def extractContentFromResponse(response: String): Option[T]

  /**
   * Common implementation for calling the LLM and processing the response. This method encapsulates the shared logic
