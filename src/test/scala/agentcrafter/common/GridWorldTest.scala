...
class GridWorldTest extends AnyFunSuite with Matchers:
  ...
  test("step moves in open space"):
    ...
    next shouldBe State(2, 1)
    reward shouldBe TEST_STEP_PENALTY

  test("step blocked by walls and boundaries"):
    val env = GridWorld(rows = TEST_GRID_ROWS, cols = TEST_GRID_COLS, walls = Set(State(2, 1)))
    env.step(State(1, 1), Action.Right).state shouldBe State(1, 1) // Blocked by wall
    env.step(State(0, 0), Action.Up).state shouldBe State(0, 2) // Toroidal wrapping: Up from row 0 goes to row 2
    env.step(State(2, 2), Action.Right).state shouldBe State(0, 2) // Toroidal wrapping: Right from col 2 goes to col 0
